...
const WEAPON_TYPES = [
  ...
  },
  {
    ...
    aspectRatio: 1.3,
    clips: [
      "polygon(50% 0%, 62% 3%, 75% 10%, 85% 22%, 90% 36%, 90% 50%, 86% 60%, 78% 66%, 72% 68%, 68% 72%, 65% 80%, 60% 88%, 55% 100%, 50% 95%, 45% 100%, 40% 88%, 35% 80%, 32% 72%, 28% 68%, 22% 66%, 14% 60%, 10% 50%, 10% 36%, 15% 22%, 25% 10%, 38% 3%)",
      "polygon(50% 0%, 64% 4%, 76% 12%, 86% 24%, 92% 38%, 91% 52%, 85% 62%, 76% 68%, 70% 70%, 66% 75%, 62% 84%, 58% 92%, 54% 100%, 50% 96%, 46% 100%, 42% 92%, 38% 84%, 34% 75%, 30% 70%, 24% 68%, 15% 62%, 9% 52%, 8% 38%, 14% 24%, 24% 12%, 36% 4%)",
    ],
    colors: [0, -1, -2, -3, 120, 80],
    glow: (hue: number) => hue < 0
      ? `0 0 20px hsl(0 0% 100% / 0.8), 0 0 45px hsl(120 60% 50% / 0.4)`
      : `0 0 15px hsl(${hue} 80% 50% / 0.8), 0 0 35px hsl(120 70% 40% / 0.5)`,
  },
];
